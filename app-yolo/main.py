...
class_labels = {0: 'mobil', 1: 'orang'}  # Adjusted mapping based on your model
...
def camera_thread(camera_index, model_param, mouse_controller_param, flip_param=False, distance_estimator_param=None):
    ...
    while True:
        ...
        frame, count_people_in_roi, vehicle_detected_in_frame = process_frame(
            frame, model_param, roi, estimate_distance=estimate_distance,
...
for index in camera_roles.keys():
    ...
    # Pass model to the camera_thread
    thread = threading.Thread(target=camera_thread, args=(index, model, mouse_controller, flip, distance_estimator))
    thread.start()
